...
long unsigned int Frame::nNextId=0; // static long unsigned int nNextId; 静态成员变量
     ...
     mpReferenceKF(static_cast<KeyFrame*>(NULL))
{
	...
	// 如果不调用thread.join()函数，当线程对象threadLeft被销毁时，程序会自动调用其析构函数，
    // 但是如果此时线程仍在运行，可能会导致程序出现未定义的行为，因此通常建议在使用线程时都要调用join()函数来等待线程执行完毕
    //等待两张图像特征点提取过程完成
    threadLeft.join();  // join是一个成员函数 等待线程结束并回收资源
    threadRight.join();
...
/**
 * @brief 将提取的ORB特征点分配到图像网格中  // 图像网格大小为48行64列  
 * 
 */
void Frame::AssignFeaturesToGrid()
{
    ...
    int nReserve = 0.5f*N/(FRAME_GRID_COLS*FRAME_GRID_ROWS);  // N:特征点个数
	//开始对mGrid这个二维数组中的每一个vector元素遍历并预分配空间
    for(unsigned int i=0; i<FRAME_GRID_COLS;i++)
        for (unsigned int j=0; j<FRAME_GRID_ROWS;j++)
            mGrid[i][j].reserve(nReserve);  // vector<std::size_t> mGrid[][] unsigend long 8byte 
 ...
 */
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit)  // Frustum:截锥体
{
    // mbTrackInView是决定一个地图点是否进行重投影的标志
    // 这个标志的确定要经过多个函数的确定，isInFrustum()只是其中的一个验证关卡。这里默认设置为否
    pMP->mbTrackInView = false;  // 初始化该标记 mbTrackInView用于表示该地图点是否在当前帧的视野范围内  
    ...
    const float viewCos = PO.dot(Pn)/dist;  // A dot B = |A||B|cos(theta)   A点积B = A模长 乘 B模长 乘 cos值
...
{
	// 存储搜索结果的vector
    vector<size_t> vIndices;  // Indice 指数
    vIndices.reserve(N);
    ...
    // 保证nMinCellX 结果大于等于0
    // mfGridElementWidthInv:1/网格单元宽度 即圆心到地图网格左下角距离/网格单元宽度 = 网格x坐标
    const int nMinCellX = max(0,(int)floor( (x-mnMinX-r)*mfGridElementWidthInv)); // floor:向下取整浮点数或双精度数
    // 图像左边界到半径为r的圆左边界的距离*每个像素可以均分多少个网格列 = 圆的左边界
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            ...
            const vector<size_t> vCell = mGrid[ix][iy];  // mGrid:每个网格内特征点的id
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                if(bCheckLevels)
                {
                    ...
                    if(kpUn.octave<minLevel)  // 跳过本次循环，进行下次循环
                        continue;
                ...
                if(distx*distx + disty*disty < r*r) // 这里改成圆形搜索区域，更合理
                    vIndices.push_back(vCell[j]);  // 通过检验，将特征点id存储在vIndices中
            }
        }
    }
    return vIndices;
...
void Frame::ComputeBoW()
{
    ...
    if(mBowVec.empty())  // 空的话继续向下执行，非空即以前已经计算过
    {
...
/**
 * @brief 用内参对特征点去畸变，结果报存在mvKeysUn中  // 纠正：相机畸变参数不属于相机的内参  
 * 
 */
void Frame::UndistortKeyPoints()  // 存储在mvKeysUn
{
    ...
    {
		//根据索引获取这个特征点
		//注意之所以这样做而不是直接重新声明一个特征点对象的目的是，能够得到源特征点对象的其他属性  // 保留其他属性，仅改变去畸变坐标
...
// 1 双目匹配 2 根据视差计算深度   最终输出右目匹配mvuRight 和 深度mvDepth信息
// 应该是左图调用该函数，计算右图中的匹配点
void Frame::ComputeStereoMatches()
{
    ...
    for(int iR = 0; iR < Nr; iR++) {
        ...
        for(int yi=minr;yi<=maxr;yi++)
            vRowIndices[yi].push_back(iR);  // 对应行号存储特征点的列号
    }
    ...
    for(int iL=0; iL<N; iL++) {
        ...
        // 计算理论上的最佳搜索范围
        const float minU = uL-maxD;  // 最大视差:焦距
        const float maxU = uL-minD;  // 最小视差:0
        ...
        if(bestDist<thOrbDist) {
			...
			IL = IL - IL.at<float>(w,w) * cv::Mat::ones(IL.rows,IL.cols,CV_32F); // 窗口中的每个元素减去正中心的元素，简单归一化，减小光照强度的影响
            ...
            for(int incR=-L; incR<=+L; incR++) {  // L:滑动窗口的范围为[-L, L]
                ...
                // sad 计算，值越小越相似
                float dist = cv::norm(IL,IR,cv::NORM_L1); // 计算两个协方差矩阵的L1范数
            ...
            // 搜索窗口越界判断
            if(bestincR==-L || bestincR==L)  // 认为在图像边缘的质量较差不做保留
                continue;
            ...
            if(disparity>=minD && disparity<maxD) {
                ...
                // 保存归一化sad最小相似度
                // Step 5. 最优视差值/深度选择.
                mvDepth[iL]=mbf/disparity; // 双目深度计算公式 fb/d
                mvuRight[iL] = bestuR;
...
//当某个特征点的深度信息或者双目信息有效时,将它反投影到三维世界坐标系中  // 注意是：世界坐标系下
cv::Mat Frame::UnprojectStereo(const int &i)
{
    ...
    /** <li> 判断这个深度是否合法 </li> <ul> */
	//（其实这里也可以不再进行判断，因为在计算或者生成这个深度的时候都是经过检查了的_不行,RGBD的不是）// 感觉就是重复判断了
    if(z>0)
    {
