...
/*
1. 维护共视帧之间的共视关系
2. 通过计算描述向量的距离，在多个关键帧中找到最匹配的特征点
3. 在闭环完成修正后，根据修正的主帧位姿修正特征点
4. 对于非关键帧，也会产生MapPoint，仅为了临时的Tracking使用
*/
...
MapPoint::MapPoint(const cv::Mat &Pos,  //地图点的世界坐标
                   ...
                   Map* pMap):          //地图点所存在的地图
    mnFirstKFid(pRefKF->mnId),              //第一次观测/生成它的关键帧 id          关键帧中的第几帧
    mnFirstFrame(pRefKF->mnFrameId),        //创建该地图点的帧ID(因为关键帧也是帧啊)  帧中的第几帧
    nObs(0),                                //被观测次数
    ...
    mpMap(pMap)                             //从属地图
{
    Pos.copyTo(mWorldPos);  // copyTo是opencv中"cv::Mat"类的成员函数，用于将一个'Mat'对象的数据复制到另一个'Mat'对象中
    //平均观测方向初始化为0
    mNormalVector = cv::Mat::zeros(3,1,CV_32F); // 32位浮点型数据
 ...
 */
MapPoint::MapPoint(const cv::Mat &Pos, Map* pMap, Frame* pFrame, const int &idxF):  // idxF是地图点在该帧特征点中的索引号
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap)
{
    ...
    //这个算重了吧  // 好像和mNormalVector这个第一步一样 但应该也不算重复 因为mNormalVector被归一化了
    cv::Mat PC = Pos - Ow;
    const float dist = cv::norm(PC);    //到相机的距离  // cv::norm默认求的是L2范数
...
{
    //TODO 为什么这里多了个线程锁
    unique_lock<mutex> lock2(mGlobalMutex);  // 全局锁
    unique_lock<mutex> lock(mMutexPos);  // 头文件中关于位姿的锁
    Pos.copyTo(mWorldPos);
}
//获取地图点在世界坐标系下的坐标
...
{
     unique_lock<mutex> lock(mMutexFeatures);  // 头文件中关于特征的锁
     return mpRefKF;
}


/*
将一个MapPoint添加到一个KeyFrame中。
当一个MapPoint在一个KeyFrame中被观测到时，会将这个KeyFrame和MapPoint在KeyFrame中的索引记录在MapPoint的mObservations中。
*/ 
/**
 * @brief 给地图点添加观测  // 该地图点被多少关键帧同时观测
 *
...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    // 如果已经添加过观测，返回
    if(mObservations.count(pKF)) // 传入一个pKF，检测mOb里面有没有，有的话退出不再进行处理
        return;
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
            ...
            mObservations.erase(pKF);  // C++ map类型提供了erase函数用于删除制定元素 按照键值进行删除
...
map<KeyFrame*, size_t> MapPoint::GetObservations() // 函数返回类型, MapPoint的成员函数
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mObservations;
}

// 被观测到的相机数目，单目+1，双目或RGB-D则+2  // 获取观测该MapPoint的数量
...
/**
 * @brief 告知可以观测到该MapPoint的Frame，该MapPoint已被删除  // 从KF中删除掉 观测数减掉 从map中删除掉
 * 
 */
void MapPoint::SetBadFlag()  // 肉体死亡
{
    map<KeyFrame*,size_t> obs;
    {
        ...
        mbBad=true;
        // 把mObservations转存到obs，obs和mObservations里存的是指针，赋值过程为浅拷贝  // 引用本身浅拷贝，本质上是在栈区开辟空间指向统一块内存空间
        obs = mObservations;
        // 把mObservations指向的内存释放，obs作为局部变量之后自动删除  // 意思是函数执行完即void大括号删除调
        mObservations.clear();
    }
...
void MapPoint::ComputeDistinctiveDescriptors()
{
	...
	for (size_t i = 0; i<N; i++)
    {
        ...
        {
            int distij = ORBmatcher::DescriptorDistance(vDescriptors[i],vDescriptors[j]);  // 计算描述子之间距离
            Distances[i][j]=distij; // ij ji结果相同 矩阵对陈
    ...
    for(size_t i=0;i<N;i++)
    {
		...
		vector<int> vDists(Distances[i].begin(), Distances[i].end());  // 利用迭代器进行拷贝
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    {
        KeyFrame* pKF = mit->first;
        cv::Mat Owi = pKF->GetCameraCenter(); // 相机中心
...
float MapPoint::GetMaxDistanceInvariance()  // invariance：不变性
{
    unique_lock<mutex> lock(mMutexPos);
 ...
 * @return int              预测的金字塔尺度
 */
int MapPoint::PredictScale(const float &currentDist, KeyFrame* pKF)  // 传入关键帧
{
    float ratio;
    {
        unique_lock<mutex> lock(mMutexPos);
 ...
 * @return int                  尺度
 */
int MapPoint::PredictScale(const float &currentDist, Frame* pF)  // 传入普通帧
{
    float ratio;
    {
        unique_lock<mutex> lock(mMutexPos);
